...
export const Card = ({
    ...
    <div
        className={`bg-bg-secondary glass rounded-[3rem] ${padding} ${hover ? 'premium-card' : ''} transition-all duration-700 flex flex-col items-stretch ${className}`}
        style={style}
    >
        {children}
    </div>
...
export const Button = ({
    ...
    return (
        <button
            ...
            className={`
                inline-flex items-center justify-center rounded-2xl font-black transition-all duration-300
...
export const Input = ({ label, error, helperText, className = '', ...props }: InputProps) => (
    <div className="w-full space-y-4">
        ...
        <div className="relative group">
            <input
                className={`
                    flex h-20 w-full rounded-[1.5rem] md:rounded-[2rem] border-2 border-transparent bg-bg-tertiary/50 dark:bg-bg-tertiary/20 backdrop-blur-xl px-10 py-5 text-xl transition-all duration-500
                    ...
                    ${error ? 'border-danger/50 focus:ring-rose-500/10 shadow-lg' : 'shadow-inner hover:bg-white dark:hover:bg-slate-800'}
                    ${className}
                `}
                {...props}
            />
...
export const Textarea = ({ label, error, helperText, className = '', ...props }: TextareaProps) => (
    <div className="w-full space-y-4">
        ...
        <textarea
            className={`
                flex min-h-[250px] w-full rounded-[1.5rem] md:rounded-[2rem] border-2 border-transparent bg-bg-tertiary/50 dark:bg-bg-tertiary/20 backdrop-blur-xl px-10 py-7 text-xl transition-all duration-500
                placeholder:text-fg-tertiary/60 focus:outline-none focus:ring-[16px] focus:ring-accent/5 focus:border-accent/40
                disabled:cursor-not-allowed disabled:opacity-50 font-black tracking-tight
                ${error ? 'border-danger/50 focus:ring-rose-500/10 shadow-lg' : 'shadow-inner hover:bg-white dark:hover:bg-slate-800'}
                ${className}
...
export const Select = ({ label, error, helperText, options, className = '', ...props }: SelectProps) => (
    <div className="w-full space-y-4">
        ...
        <div className="relative">
            <select
                className={`
                    flex h-20 w-full rounded-[1.5rem] md:rounded-[2rem] border-2 border-transparent bg-bg-tertiary/50 dark:bg-bg-tertiary/20 backdrop-blur-xl px-10 py-5 text-lg transition-all duration-500
                    appearance-none focus:outline-none focus:ring-[16px] focus:ring-accent/5 focus:border-accent/40
                    ${error ? 'border-danger/50 shadow-lg' : 'shadow-inner hover:bg-white dark:hover:bg-slate-800'}
                    ${className}
            ...
            >
                {options?.map((option) => (
                    <option key={option.value} value={option.value} className="bg-white dark:bg-slate-900 text-fg-primary font-bold">
                        {option.label}
                    </option>
                ))}
                {props.children}
    ...
    <div className="flex flex-wrap gap-4 p-3 glass rounded-[2rem] mb-12 w-fit border border-accent/10 shadow-2xl">
        {tabs.map((tab: any) => (
            <button
                ...
                className={`
                    px-10 py-4.5 text-xs font-black uppercase tracking-widest rounded-[1.25rem] transition-all duration-700
                    ${activeTab === tab.id
