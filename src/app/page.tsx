          ...
          <div className="max-w-[1200px] mx-auto text-center">
            ...
            <Reveal delay={0.2}>
              <div className="max-w-4xl mx-auto relative group">
                <div className="absolute -inset-2 bg-accent/20 rounded-[3rem] blur-3xl opacity-0 group-hover:opacity-100 transition duration-300" />
                <Magnetic>
                  <div className="relative glass p-6 rounded-[3rem] shadow-[0_80px_160px_-40px_rgba(37,99,235,0.2)] border-2 border-accent/10 hover:border-accent/30 transition-all duration-150">
                    <SearchBar />
                  </div>
                </Magnetic>
        ...
        <div className="page-container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {categories.map((category, index) => (
              <Reveal key={category.id} delay={index * 0.02}>
                <div className="premium-card p-12 group h-full flex flex-col border-2 border-transparent">
                  <div className="absolute top-0 right-0 p-12 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <category.icon size={220} className="text-accent opacity-[0.03] rotate-12" />
                  </div>

                  <div className="flex items-start justify-between mb-16">
                    <Magnetic>
                      <div className="w-20 h-20 rounded-3xl bg-accent text-white flex items-center justify-center shadow-2xl shadow-accent/40 group-hover:rotate-6 transition-all duration-150">
                  ...
                  <div className="space-y-5 mb-12 mt-6">
                    {category.tools.slice(0, 4).map((tool) => (
                      <Link
                        key={tool.name}
                        href={tool.href}
                        className="flex items-center group/item hover:translate-x-2 transition-transform duration-150"
                      >
